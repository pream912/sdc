<template>
    <v-container>
        ...
        <v-row>
            <!-- <v-col cols="12">
                <v-autocomplete @change="getMoreData" outlined label="Customer name" item-value="id" v-model="cid" item-text="name" :items="customers">
            ...
            <v-col v-if="items1.length > 0" cols="12">
                ...
                :items="items1">
                    <template v-slot:[`item.actions`]="{ item }">
                        <v-btn x-small @click="addPayment(item.invno)" v-if="item.balance > 0"  color="green">Add payment</v-btn>
                    </template>
                </v-data-table>
            </v-col>
        </v-row>
...
export default {
    data: () => ({
        ...
        duraList: ['Current month', 'Last 7 days', 'Select range', 'All'],
    ...
    methods: {
        getData() {
            ...
            this.items = cdata.filter((item) => {
                return item.pamount != 0
            })
        },
        ...
        addPayment(invno) {
            ...
            this.selectedInv = this.invoices.find((item) => {
                return item.id == invno
            })
